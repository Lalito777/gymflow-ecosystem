...
class QRServiceTest {
    ...
    void generateQR_conContenidoValido_deberiaGenerarImagenBase64YGuardar() throws Exception {
        ...
        // When
        QRData result = qrService.generateQR(1L, "TOKEN-ABC-123");

        // Then
        assertNotNull(result);
        assertEquals(1L, result.getAccessTokenId());
        assertEquals("TOKEN-ABC-123", result.getContenidoQr());
        assertTrue(result.getImagenBase64().startsWith("data:image/png;base64,"));
    ...
    void generateQR_conOtroAccessTokenId_deberiaPreservarElIdRecibido() throws Exception {
        ...
        // When
        QRData result = qrService.generateQR(42L, "TOKEN-XYZ-999");

        // Then
        assertEquals(42L, result.getAccessTokenId());
        assertEquals("TOKEN-XYZ-999", result.getContenidoQr());
        verify(repo, times(1)).save(any(QRData.class));
    }
}
